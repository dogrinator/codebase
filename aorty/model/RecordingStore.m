classdef RecordingStore < handle
    ...
    methods
        ...
        function finalize(store, status, reason, integrity)
            ...
            if isempty(store.pendingFinalMetadata)
                if nargin < 4 || isempty(integrity)
                    integrity = struct( ...
                        'droppedSamples', struct('X', 0, 'Y', 0), ...
                        'restartDetected', false);
                end
                store.pendingFinalMetadata = struct( ...
                    'status', lower(char(status)), ...
                    'reason', char(reason), ...
                    'endTime', datetime('now'), ...
                    'cameraRecordCount', store.cameraRecordCount, ...
                    'axisRecordCounts', store.axisRecordCounts, ...
                    'integrity', integrity);
            end
            metadata = store.pendingFinalMetadata;
            store.flush();
            store.close();

            % Write final metadata after append handles are closed. Attribute
            % writes are idempotent, so a transient failure can be retried
            % without changing the original stop time or completion details.
            RecordingStore.writeFixedTextAttribute( ...
                store.h5Path, '/metadata', 'status', metadata.status, ...
                store.TEXT_STATUS_LENGTH);
            RecordingStore.writeFixedTextAttribute( ...
                store.h5Path, '/metadata', 'reason', metadata.reason, ...
                store.TEXT_REASON_LENGTH);
            RecordingStore.writeFixedTextAttribute( ...
                store.h5Path, '/metadata', 'end_time', ...
                RecordingStore.formatTime(metadata.endTime), ...
                store.TEXT_TIME_LENGTH);
            h5writeatt(store.h5Path, '/metadata', ...
                'camera_record_count', ...
                uint64(metadata.cameraRecordCount));
            h5writeatt(store.h5Path, '/metadata', ...
                'x_sample_count', ...
                uint64(metadata.axisRecordCounts.X));
            h5writeatt(store.h5Path, '/metadata', ...
                'y_sample_count', ...
                uint64(metadata.axisRecordCounts.Y));
            h5writeatt(store.h5Path, '/metadata', ...
                'x_dropped_sample_count', ...
                uint64(metadata.integrity.droppedSamples.X));
            h5writeatt(store.h5Path, '/metadata', ...
                'y_dropped_sample_count', ...
                uint64(metadata.integrity.droppedSamples.Y));
            h5writeatt(store.h5Path, '/metadata', ...
                'sample_loss_detected', uint8( ...
                metadata.integrity.droppedSamples.X > 0 || ...
                metadata.integrity.droppedSamples.Y > 0));
            h5writeatt(store.h5Path, '/metadata', ...
                'plc_restart_detected', ...
                uint8(logical(metadata.integrity.restartDetected)));
            store.finalized = true;
            store.pendingFinalMetadata = [];
        end
