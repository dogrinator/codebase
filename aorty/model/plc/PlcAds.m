classdef PlcAds < handle
    ...
    methods
        ...
        function initialize(ads, testing)
            ...
            try
                ...
                ads.handles.persistence = struct( ...
                    'saved', ads.makeHandle( ...
                        'MAIN.bPersistentPositionSaved'), ...
                    'busy', ads.makeHandle( ...
                        'MAIN.bPersistentPositionSaveBusy'), ...
                    'error', ads.makeHandle( ...
                        'MAIN.bPersistentPositionSaveError'), ...
                    'errorID', ads.makeHandle( ...
                        'MAIN.nPersistentPositionSaveErrorID'), ...
                    'counter', ads.makeHandle( ...
        ...
        function state = readPersistentPositionCheckpoint(ads)
            handles = ads.handles.persistence;
            state = struct( ...
                'saved', ads.readBool(handles.saved), ...
                'busy', ads.readBool(handles.busy), ...
                'error', ads.readBool(handles.error), ...
                'errorID', ads.readUdint(handles.errorID), ...
                'counter', ads.readUdint(handles.counter));
        end
    ...
    methods (Access = private)
        ...
        function value = readBool(ads, handle)
            if ismethod(ads.client, 'ReadStatusPacket')
                value = logical(ads.client.ReadAny(handle));
            else
                value = logical(ads.client.ReadAny( ...
                    handle, System.Type.GetType('System.Boolean')));
            end
        end

        function value = readUdint(ads, handle)
            if ismethod(ads.client, 'ReadStatusPacket')
                value = uint32(ads.client.ReadAny(handle));
            else
                value = uint32(ads.client.ReadAny( ...
