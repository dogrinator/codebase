    ...
    methods (Static)
        ...
        function recording = readRecording(folderPath)
            ...
            try
                ...
                plcInterval = double(h5readatt( ...
                    h5File, '/metadata', 'plc_interval_seconds'));
        ...
        function data = sampleTable( ...
            ...
            if ~isempty(values) && any(diff(values(1, :)) < 0)
                ...
                % samples themselves remain valid and in sequence.
                values(1, :) = values(1, 1) + ...
                    (0:size(values, 2) - 1) * plcInterval;
                warning('PostProcessor:RecoveredTimestampOrder', ...
                    ['%s-axis samples contained non-chronological ' ...
