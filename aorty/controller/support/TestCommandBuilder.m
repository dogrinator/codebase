classdef TestCommandBuilder
    ...
    methods (Static, Access = private)
        ...
        function tolerance = percentTolerance(percent, hwConfig, field)
            axisName = [lower(field), 'Axis'];
            if ~isstruct(hwConfig) || ~isfield(hwConfig, 'plc') || ...
                    ~isfield(hwConfig.plc, axisName) || ...
                    ~isfield(hwConfig.plc.(axisName), 'fMaxForce')
                error('Control:MissingMaximumForce', ...
                    'Maximum force is missing for the %s axis.', ...
                    upper(field));
            end
            percent = double(percent);
            maxForce = double(hwConfig.plc.(axisName).fMaxForce);
            if ~isscalar(percent) || ~isfinite(percent) || ...
                    percent < 0 || percent > 100 || ...
                    ~isscalar(maxForce) || ~isfinite(maxForce) || ...
                    maxForce <= 0
                error('Control:InvalidTolerancePercent', ...
                    ['Tolerance must be from 0 to 100%% and maximum force ' ...
                    'must be positive for the %s axis.'], upper(field));
            end
            tolerance = percent * maxForce / 100;
        end
