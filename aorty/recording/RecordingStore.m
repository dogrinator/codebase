classdef RecordingStore < handle
    ...
    methods
        ...
        function store = RecordingStore(folderPath, header)
            ...
            if ~isempty(existing)
                error('RecordingStore:RecordingFilesExist', ...
            ...
            try
                ...
                if store.cameraFid == -1
                    error('RecordingStore:RecordingOpenFailed', ...
        ...
        function appendAxis(store, axisName, timestamps, forceValues, untaredForceValues, positionValues)
            ...
            if ~ismember(axisName, {'X', 'Y'})
                error('RecordingStore:RecordingAxis', ...
            ...
            if any(counts ~= counts(1))
                error('RecordingStore:RecordingVectorLength', ...
            ...
            if any(~isfinite(values), 'all')
                error('RecordingStore:RecordingValues', ...
        ...
        function appendFrame(store, frame, timestamp, index, systemStatus)
            ...
            if store.expectedFrameBytes > 0 && ...
                ...
                error('RecordingStore:CameraFrameSize', ...
            ...
            if ~ismember(int16(systemStatus), validStatuses)
                error('RecordingStore:InvalidSystemStatus', ...
            ...
            if written ~= numel(frame)
                error('RecordingStore:RecordingWriteFailed', ...
    ...
    methods (Access = private)
        ...
        function requireOpen(store)
            if store.closed
                error('RecordingStore:RecordingClosed', ...
    ...
    methods (Static, Access = private)
        ...
        function writeStructAttributes(filename, target, values)
            ...
            for index = 1:numel(names)
                ...
                if isstruct(value)
                    error('RecordingStore:RecordingHeader', ...
