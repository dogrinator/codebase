...
settings.appConfig.single.rupture.enabled = true;
settings.appConfig.single.rupture.value = 12;
single = TestCommandBuilder.fromPreset( ...
    settings.appConfig, 'single', settings.hwConfig);
verifyEqual(testCase, single.X.stop2Mode, 3);
verifyEqual(testCase, single.X.stop2Value, 12);
...
verifyEqual(testCase, [preview.X.tolerance], ...
    max(abs([12, 8, 2]) * config.pre.forceTolerance.x / 100, ...
...
verifyEqual(testCase, preview.X.tolerance, ...
    max(abs(config.single.secondary.x) * ...
