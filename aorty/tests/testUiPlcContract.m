...
verifyNotEmpty(testCase, strfind(testUiSource, ...
    'numericCycleRow'));
verifyNotEmpty(testCase, strfind(factorySource, ...
    '''Limits'', [1, 50]'));
verifyNotEmpty(testCase, strfind(factorySource, ...
...
verifyNotEmpty(testCase, strfind(testUiSource, ...
    '''TIFF sampling period'''));
verifyNotEmpty(testCase, strfind(testUiSource, ...
    '''Include pre-test frames'''));
verifyEmpty(testCase, strfind(testUiSource, ...
    '''Include pre/post-test frames'''));
verifyEmpty(testCase, strfind(testUiSource, ...
...
verifyEmpty(testCase, strfind(tabsSource, '[mm / N]'));
verifyEmpty(testCase, strfind(tabsSource, '''[mm/s]'''));
verifyNotEmpty(testCase, strfind(factorySource, '\frac'));
verifyNotEmpty(testCase, strfind(factorySource, '\mathrm{s}^{2}'));
end
...
verifyNotEmpty(testCase, strfind(viewSource, ...
    'loadStartupDefaults'));
% Recording lifecycle tests verify the post-processing settings passed to
% Control behaviorally; do not couple this UI contract to line wrapping.
