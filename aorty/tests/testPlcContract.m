...
requiredCommand = {'fMoveVelocity', 'fTargetForce', ...
...
movement = fileread(fullfile(plcRoot, 'POUs', ...
    'fb_MovementController.TcPOU'));
expected = {
    'fMoveVelocity\s*:\s*LREAL'
    'nJogLeaseCounter\s*:\s*UDINT'
...
verifyNotEmpty(testCase, regexp(statusDut, ...
    'nInterfaceVersion\s*:\s*UDINT\s*:=\s*7', 'once'));
verifyNotEmpty(testCase, regexp(statusDut, ...
...
verifyNotEmpty(testCase, strfind(movement, ...
    'ERROR_JOG_LEASE_EXPIRED : UDINT := 2011'));
verifyNotEmpty(testCase, strfind(movement, ...
    'ERROR_VELOCITY_LIMIT : UDINT := 2012'));
verifyNotEmpty(testCase, strfind(movement, ...
    'STATE_FORCE_READY_WAIT_STOP : INT := 316'));
...
verifyNotEmpty(testCase, strfind(movement, ...
    'ABS(fStop2CurrentError) <='));
verifyLessThan(testCase, strfind(movement, ...
    'IF bSinglePrimaryActive AND stMoveCommand.nStop2Mode > 0 THEN'), ...
    strfind(movement, 'positionEndpoint();'));
verifyNotEmpty(testCase, strfind(movement, ...
    'ABS(stMoveCommand.fStop2Value) *'));
verifyNotEmpty(testCase, strfind(movement, ...
    'FORCE_REG_MIN_SPEED_FRACTION : LREAL := 0.01'));
verifyNotEmpty(testCase, strfind(movement, ...
    'DISPLACEMENT_POLARITY : INT := -1'));
verifyEqual(testCase, count(movement, ...
    'INT_TO_LREAL(DISPLACEMENT_POLARITY)'), 4);
verifyNotEmpty(testCase, regexp(movement, ...
    'fForceRegMaxVelocity\s*\*\s*FORCE_REG_MIN_SPEED_FRACTION', ...
    'once'));
verifyEqual(testCase, count(movement, ...
    'nVelocityUpdateCounter < VELOCITY_UPDATE_CYCLES'), 2);
verifyNotEmpty(testCase, strfind(movement, 'fSinglePeakForce'));
verifyNotEmpty(testCase, strfind(movement, 'fSingleStartForce'));
verifyNotEmpty(testCase, strfind(movement, 'bSingleRuptureArmed'));
verifyNotEmpty(testCase, strfind(movement, ...
    'fSingleStartForce + stSettings.fForceTolerance'));
verifyNotEmpty(testCase, strfind(movement, ...
    '1.0 - stMoveCommand.fStop2Value / 100.0'));
verifyNotEmpty(testCase, strfind(movement, 'bZeroForceUnload'));
end
...
verifyNotEmpty(testCase, strfind(main, ...
    'nPersistentCheckpointRequestGeneration'));
verifyNotEmpty(testCase, strfind(main, ...
    'bReferenceValid := fbSafetyX.bHomed'));
verifyNotEmpty(testCase, strfind(main, ...
    'bReferenceValid := fbSafetyY.bHomed'));
end
...
function testTenMillisecondTimingContract(testCase)
task = fileread(fullfile(testCase.TestData.plcRoot, 'PlcTask.TcTTO'));
movement = fileread(fullfile(testCase.TestData.plcRoot, 'POUs', ...
    'fb_MovementController.TcPOU'));
taskMicros = str2double(regexp(task, ...
    '<CycleTime>(\d+)</CycleTime>', 'tokens', 'once'));
plcSeconds = str2double(regexp(movement, ...
    'CYCLE_TIME\s*:\s*LREAL\s*:=\s*([0-9.]+)', ...
    'tokens', 'once'));
verifyEqual(testCase, taskMicros / 1e6, plcSeconds, ...
    'AbsTol', eps);
verifyEqual(testCase, AppInfo.PLC_SAMPLE_PERIOD_SECONDS, ...
    plcSeconds, 'AbsTol', eps);
end
